...
export default function SalaryRuleModal({
    ...
    const currentSubRoleOptions = useMemo(() => {
        if (!form.role) return []
        return SUB_ROLE_OPTIONS[form.role as keyof typeof SUB_ROLE_OPTIONS] || []
    }, [form.role])
    ...
    const addComponent = (id: string) => {
        ...
        setForm(prev => {
            if (prev.components.find(c => c.id === numId)) return prev
            return {
                ...prev,
                components: [
                    ...prev.components,
            ...
            <DialogContent className="sm:max-w-md max-h-[90vh] overflow-y-auto">
                ...
                <div className="flex flex-col gap-4 w-full">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-medium">Pilih Role Utama</label>
                    ...
                    {/* SUB ROLE (Dinamis) */}
                    <div className="space-y-1">
                        <label className="text-xs font-medium">Sub Role / Kategori</label>
                        <Select
                            value={form.sub_role}
                            onValueChange={(v) => setForm(prev => ({ ...prev, sub_role: v }))}
                            disabled={!form.role}
                        >
                            <SelectTrigger>
                                <SelectValue placeholder={form.role ? "Pilih Sub Role" : "Pilih role terlebih dahulu"} />
                            </SelectTrigger>
                            <SelectContent>
                    ...
                    {/* ADD COMPONENT */}
                    <div className="space-y-1">
                        <label className="text-xs font-medium">Komponen Gaji</label>
                    ...
                    <div className="space-y-3">
                        {form.components.map((c) => {
                            ...
                            return (
                                <div key={c.id} className="border rounded p-3 space-y-2 bg-slate-50/50">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        <Select
                                            value={c.amount_type}
                                            onValueChange={(v) => updateComponent(c.id, 'amount_type', v)}
                                        >
                                            ...
                                            <SelectContent>
                                                ...
                                                <SelectItem value="formula">Formula (Kehadiran)</SelectItem>
                                            </SelectContent>
                                        </Select>
                                        ...
                                        <Input
                                            ...
                                            value={c.amount}
                                            onChange={(e) => updateComponent(c.id, 'amount', Number(e.target.value))}
                                            placeholder={c.amount_type === 'formula' ? "Tarif/Hadir" : "Nilai"}
                                        />
                                    </div>
                                    ...
                                    {c.amount_type === 'formula' && (
                                        <p className="text-[10px] text-blue-600 italic leading-tight">
                                            * Otomatis: (Nominal × Total status 'hadir' di absensi)
                                        </p>
                                    )}
                                </div>
                            )
                    ...
                    {/* STATUS */}
                    <div className="space-y-1">
                        <label className="text-xs font-medium">Status Rule</label>
                        <Select
                            value={form.is_active ? '1' : '0'}
                            onValueChange={(v) => setForm(prev => ({ ...prev, is_active: v === '1' }))}
                        >
                            <SelectTrigger>
